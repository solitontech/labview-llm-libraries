...
const knowledgeBase = [
  {
    message: "LabVIEW is a system-design platform and development environment for a visual programming language from National Instruments.",
    source: "https://www.ni.com/en/shop/labview.html"
  },
  {
    message: "Large Language Models (LLMs) are advanced AI models trained on vast amounts of text data that can generate human-like text responses.",
    source: "https://openai.com/research/language-models"
  },
  {
    message: "Vector databases are specialized databases designed to store and search high-dimensional vectors, commonly used for semantic search and AI applications.",
    source: "https://qdrant.tech/documentation/"
  },
  {
    message: "Embedding models convert text or other data types into numerical vector representations that capture semantic meaning.",
...
function generateMockResponse(message) {
  ...
  const matches = knowledgeBase.filter(item => 
    ...
    lowercaseMessage.split(' ').some(word => 
      word.length > 3 && item.message.toLowerCase().includes(word)
    )
  );
